...
USB External Camera는 UVC 호환 USB 외부 카메라를 사용하여 iPad 최적화 라이브 스트리밍을 제공하는 전문 iOS 애플리케이션입니다. HaishinKit 2.2.5 기반으로 YouTube Live 실시간 RTMP 송출을 지원하며, 16:9 가로·9:16 세로 양방향 480p/720p/1080p 프리셋, 화면 캡처 기반 합성(카메라 + UI), 안정성 중심 프레임 파이프라인 제어를 제공합니다.
...
- 📡 **실시간 RTMP** - HaishinKit 2.2.5 기반 전문 스트리밍
- 🎛️ **고급 제어** - 비디오/오디오 품질, 해상도, 비트레이트 설정
- 🎚️ **YouTube 프리셋** - 480p / 720p / 1080p 빠른 프리셋 + 수동 설정 지원
...
- ✅ 라이브 시작 전 마이크 권한/오디오 세션 사전 점검 (오디오 0 kbps 방지)
- ✅ 화면 캡처 파이프라인 프레임 백프레셔 제어 (버퍼링/끊김 완화)
- ✅ 실시간 마이크 음소거 토글 — MediaMixer 오디오 파이프라인에 즉시 반영되어 송출에도 적용됩니다

### 비디오 및 오디오
- 🎬 **해상도**: 480p (848×480), 720p (1280×720), 1080p (1920×1080)
- 🎵 **오디오**: 설정 가능한 비트레이트로 AAC 인코딩
- 📏 **프레임 레이트**: UI에서 24/30/60 선택 가능 (실제 캡처는 현재 최대 30fps, 1080p는 24fps 최적화 적용)
- 🔧 **비트레이트 제어**: 비디오 500-10000 kbps, 오디오 64-256 kbps

### 고급 기능
- 📈 실시간 스트리밍 통계 및 모니터링
- 🔄 자동 재연결 및 오류 복구
- ⚙️ YouTube Live 최적화 프리셋 (480p/720p/1080p)
- 🎯 네트워크 품질 모니터링 및 비트레이트 조정
- 🛡️ 블랙 프레임 완화 (UI-only fallback + stale 프레임 재사용)
- 📱 iOS 기기 방향 지원
- ⚡ iPad 기기 성능 최적화
...
- **iOS**: 17.0 이상
- **Xcode**: 16.3 이상
- **개발 환경**: 로컬 빌드 및 시뮬레이터 검증은 Apple Silicon Mac 기준으로 공식 지원
- **기기**: USB-C 포트가 있는 iPad (iPhone은 공식 지원하지 않음 - 테스트 단말 부족으로 검증되지 않음)
- **카메라**: UVC 호환 USB 외부 카메라
- **마이크**: 오디오 트랙 송출을 위해 마이크 권한 필요
- **네트워크**: 안정적인 업로드 대역폭 (권장: 480p 3+ Mbps, 720p 6+ Mbps, 1080p 10+ Mbps)
- **화면 방향**: **가로(16:9)** 와 **세로(9:16)** 모두 지원. 송출 방향은 설정에서 선택할 수 있으며, 대기 상태에서는 디바이스 방향에 자동 동기화됩니다.

> **정책 참고**: Intel Mac 개발 환경은 공식 지원 대상이 아닙니다. 시뮬레이터 검증은 Apple Silicon Mac 기준으로 운영하며, 이 프로젝트는 의도적으로 `x86_64` iOS Simulator 아키텍처를 제외합니다.
...
# 기기에서 빌드 및 실행
# 참고: 카메라 기능을 위해서는 실제 기기가 필요합니다
# 참고: Intel Mac 시뮬레이터 빌드는 프로젝트 정책상 지원하지 않습니다
```
...
- [HaishinKit](https://github.com/HaishinKit/HaishinKit.swift) (2.2.5, `HaishinKit` + `RTMPHaishinKit` products) - RTMP 스트리밍 엔진
...
3. 스트리밍 품질 프리셋 선택:
   ...
   - **1080p**: 고성능 iPad + 안정적인 업링크 환경에서 권장 (권장 비트레이트: 4500-9000 kbps)
...
**레이아웃 옵션:**
- **와이드 화면(iPad/Mac)**: 카메라 프리뷰(45%) + YouTube Studio(55%) 좌우 배치
- **좁은 화면(iPhone)**: 카메라 프리뷰(35%) + YouTube Studio(60%) 상하 배치

**WebView 개선 사항:**
- 최신 YouTube Studio 기능 호환을 위한 최신 User-Agent 적용
- 전체 기능 동작을 위한 JavaScript 처리 강화
- 로그인 세션 자동 데이터 유지
- 커스텀 액세서리 뷰를 통한 키보드 입력 처리 개선
...
| Full HD (1080p) | 1080p | 4.5 Mbps (권장 4.5-9.0 Mbps) | 128 kbps | 목표 30 fps (캡처는 24 fps 최적화) |
...
- 1080p는 6000-6800 kbps부터 시작하고 YouTube 상태를 보며 점진적으로 상향
- 버퍼링이 지속되면 비트레이트를 다시 올리기 전에 720p 프리셋으로 전환

**YouTube 송출 화면이 검은 화면으로 나옴**
- 스트리밍 시작 전에 앱 내 카메라 프리뷰가 정상 표시되는지 먼저 확인
- 마이크 권한을 유지 (오디오 트랙 초기화가 시작 단계에 포함됨)
- 프리뷰가 멈추면 카메라/허브를 재연결 (고전력 카메라는 전원형 USB-C 허브 권장)
- 해상도/비트레이트 등 주요 설정 변경 후에는 한 번 중지/재시작 후 재확인

**YouTube 경고: 현재 비트레이트 낮음 / 오디오 0 kbps**
- 순간 속도보다 지속 업로드 대역폭 기준으로 네트워크 상태 확인
- 오디오 비트레이트를 최소 **128 kbps** 이상으로 설정
- iPad 설정 > 개인정보 보호 및 보안 > 마이크에서 권한 허용
- 영상은 정상인데 경고가 계속되면 비디오 비트레이트를 먼저 낮춘 뒤 재테스트

**성능 최적화**
- 1080p를 지원하며, 버퍼링 완화를 위해 화면 캡처 파이프라인에서 24fps 캡처 최적화를 적용
- 느린 기기나 제한된 대역폭에는 480p 권장
- 사용 가능한 경우 하드웨어 가속 자동 활성화

**가로 / 세로 방향 이슈**
- 가로·세로 두 방향 모두 지원됩니다. 송출 중이 아닐 때는 디바이스 방향에 따라 16:9 또는 9:16 비율이 자동으로 동기화됩니다.
- 특정 방향으로 강제하려면 설정 → 송출 방향에서 변경하세요. 방송 중에는 방향이 잠깁니다 — 송출을 멈춘 뒤 다시 선택해 주세요.
...
# 참고: Intel Mac 시뮬레이터 검증은 지원하지 않음
```

### 🚧 향후 개발 계획

- **MediaMixer 통합**: 화면 캡처용 `mixer` 와 Examples 패턴 `mediaMixer` 두 인스턴스를 단일 라이프사이클로 정리 (PR #17 리뷰 follow-up).
- **향상된 UI 레이아웃**: 다양한 화면 방향에 대한 반응형 디자인 개선
- **추가 카메라 기능**: 더 많은 카메라 제어 및 설정 옵션
- **성능 개선**: `ManualFrameProcessor` 의 `CVPixelBufferPool` 재사용 및 Metal 기반 color/scale 변환 도입 (LSC 에서 추적).
