...
final actor StreamSwitcher {
    ...
    func startStreaming() async throws {
        ...
        do {
            ...
            // 🔍 스트림 키 상세 검증 및 정제
            let cleanedStreamKey = await haishinKitManager?.cleanAndValidateStreamKey(preference.streamKey) ?? preference.streamKey
...
public class HaishinKitManager: NSObject, @preconcurrency HaishinKitManagerProtocol, ObservableObject, CameraFrameDelegate {
    ...
    private func checkConnectionHealth() async {
        ...
        if let connection = await streamSwitcher.connection {
            ...
            if !isReallyStreaming {
                ...
                logger.warning("⚠️ 연결 상태 불량 감지 - 연결: \(isConnected), 퍼블리싱: \(isStreamPublishing) (\(connectionFailureCount)/\(maxConnectionFailures))", category: .connection)
    ...
    private func checkScreenCapturePermissions() {
        ...
        if let settings = currentSettings {
            logger.error("    📊 현재 설정 해상도: \(settings.videoWidth)x\(settings.videoHeight)", category: .connection)
    ...
    private func analyzeDataFlowConnection() {
        ...
        logger.error("      • 현재 FPS: \(String(format: "%.1f", screenCaptureStats.currentFPS))", category: .connection)
    ...
    private func logDetailedTransmissionStats() {
        ...
        if stats.droppedFrames > 0 || stats.connectionQuality == .poor {
            logger.warning("⚠️ 스트림 품질 문제: 드롭 프레임 \(stats.droppedFrames)개, 품질: \(stats.connectionQuality.description)", category: .streaming)
        }
        
        // 주요 이정표 프레임 수에서만 간단 요약 로그 (1000 프레임마다)
        if stats.videoFramesTransmitted > 0 && stats.videoFramesTransmitted % 1000 == 0 {
            logger.info("📊 스트림 요약: \(stats.videoFramesTransmitted)프레임 전송, 평균 \(String(format: "%.1f", stats.averageFrameRate))fps", category: .streaming)
        }
    }
    ...
    private func attemptReconnection() async {
        ...
        do {
            ...
            if reconnectAttempts >= maxReconnectAttempts {
                ...
                currentStatus = .error(LiveStreamError.networkError("재연결에 실패했습니다. 네트워크 상태를 확인 후 수동으로 다시 시작해주세요."))
    ...
    public func loadSettings() -> USBExternalCamera.LiveStreamSettings {
        ...
        if let streamKey = KeychainManager.shared.loadStreamKey(), !streamKey.isEmpty {
            ...
            if let legacyStreamKey = defaults.string(forKey: "LiveStream.streamKey"), !legacyStreamKey.isEmpty {
    ...
    private func isSettingsEqual(_ settings1: USBExternalCamera.LiveStreamSettings, _ settings2: USBExternalCamera.LiveStreamSettings) -> Bool {
    ...
    private func analyzeStreamKeyIssues(for settings: USBExternalCamera.LiveStreamSettings) {
        ...
        logger.error("    🔤 스트림 키 형식: \(cleanedKey.prefix(4))***\(cleanedKey.suffix(2))", category: .connection)
        ...
        if settings.rtmpURL.contains("youtube.com") {
            ...
            let isValidYouTubeFormat = regex?.firstMatch(in: cleanedKey, range: NSRange(location: 0, length: cleanedKey.count)) != nil
    ...
    private func setupScreenCaptureMediaMixer() async throws {
        ...
        if let settings = currentSettings {
            logger.info("📹 화면 캡처용 목표 해상도: \(settings.videoWidth)x\(settings.videoHeight) @ \(settings.videoBitrate)kbps", category: .system)
    ...
    private func applyStreamSettings() async throws {
        ...
        logger.info("  📺 비디오: \(settings.videoWidth)×\(settings.videoHeight) @ \(settings.videoBitrate) kbps", category: .system)
    ...
    /// 스트리밍 설정에 맞춰 오디오 디바이스 최적화
    private func optimizeAudioDevice(_ audioDevice: AVCaptureDevice, for settings: USBExternalCamera.LiveStreamSettings) throws {
    ...
    private func preprocessPixelBuffer(_ pixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
        ...
        logger.info("   • 현재: \(originalWidth)x\(originalHeight) (비율: \(String(format: "%.2f", originalAspectRatio)))")
        logger.info("   • 목표: \(targetWidth)x\(targetHeight) (비율: \(String(format: "%.2f", targetAspectRatio)))")
        ...
        if originalWidth != targetWidth || originalHeight != targetHeight {
            logger.info("🔄 고품질 캡처 → 송출 해상도 스케일링: \(originalWidth)x\(originalHeight) → \(targetWidth)x\(targetHeight)")
        } else {
        ...
        if finalWidth == targetWidth && finalHeight == targetHeight {
            ...
            logger.error("❌ 해상도 스케일링 검증 실패: 목표 \(targetWidth)x\(targetHeight) vs 결과 \(finalWidth)x\(finalHeight)")
            return formatCompatibleBuffer // 검증 실패 시 포맷만 변환된 버퍼 반환
        }
    }
    
    /// CVPixelBuffer 해상도 스케일링 (고품질, HaishinKit 최적화, VideoCodec 호환성 보장)
    private func scalePixelBuffer(_ pixelBuffer: CVPixelBuffer, to targetSize: CGSize) -> CVPixelBuffer? {
        ...
        if finalWidth != requestedWidth || finalHeight != requestedHeight {
            logger.info("📐 해상도 16의 배수 정렬: \(requestedWidth)x\(requestedHeight) → \(finalWidth)x\(finalHeight)")
        } else {
        ...
        do {
        context.render(scaledImage, to: scaledBuffer, bounds: targetRect, colorSpace: CGColorSpace(name: CGColorSpace.sRGB)!)
        } catch {
        ...
        if resultWidth == finalWidth && resultHeight == finalHeight {
            let originalInputRatio = Double(CVPixelBufferGetWidth(pixelBuffer)) / Double(CVPixelBufferGetHeight(pixelBuffer))
            let finalOutputRatio = Double(finalWidth) / Double(finalHeight)
            
            logger.info("✅ Aspect Fill 스케일링 성공:")
            logger.info("   • 입력: \(CVPixelBufferGetWidth(pixelBuffer))x\(CVPixelBufferGetHeight(pixelBuffer)) (비율: \(String(format: "%.2f", originalInputRatio)))")
            logger.info("   • 출력: \(finalWidth)x\(finalHeight) (비율: \(String(format: "%.2f", finalOutputRatio)))")
            logger.info("   • 1:1 → 16:9 변환: \(abs(originalInputRatio - 1.0) < 0.1 ? "✅완료" : "N/A")")
            return scaledBuffer
        } else {
            logger.error("❌ 스케일링 결과 불일치: 예상 \(finalWidth)x\(finalHeight) vs 실제 \(resultWidth)x\(resultHeight)")
            return nil
        }
    }
    ...
    private func createSampleBuffer(from pixelBuffer: CVPixelBuffer) -> CMSampleBuffer? {
        ...
        if CMSampleBufferIsValid(finalBuffer) {
            ...
            // CVPixelBuffer 재확인
            guard let _ = CMSampleBufferGetImageBuffer(finalBuffer) else {
    ...
    private func convertToSupportedFormat(_ pixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
        ...
        logger.info("🔄 [convertToSupportedFormat] BGRA→YUV420 변환: \(currentFormat) → \(targetFormat) (\(width)x\(height))")
    ...
    private func convertBGRAToYUV420UsingvImage(sourceBuffer: CVPixelBuffer, destinationBuffer: CVPixelBuffer) -> Bool {
        ...
        guard let yPlaneAddress = CVPixelBufferGetBaseAddressOfPlane(destinationBuffer, 0),
              let uvPlaneAddress = CVPixelBufferGetBaseAddressOfPlane(destinationBuffer, 1) else {
    ...
    private func convertBGRAToYUV420UsingCIImage(_ pixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
        ...
        do {
            context.render(inputImage, to: outputBuffer, bounds: targetRect, colorSpace: CGColorSpace(name: CGColorSpace.itur_709)!)
            return outputBuffer
        } catch {
    ...
    private func convertToBGRAFormat(_ pixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
        ...
        context.render(inputImage, to: outputBuffer, bounds: targetRect, colorSpace: CGColorSpace(name: CGColorSpace.sRGB)!)
    ...
    private func scalePixelBuffer(_ pixelBuffer: CVPixelBuffer, toWidth newWidth: Int, toHeight newHeight: Int) -> CVPixelBuffer? {
        let originalWidth = CVPixelBufferGetWidth(pixelBuffer)
        let originalHeight = CVPixelBufferGetHeight(pixelBuffer)
    ...
    private func setupLocalComponentsInParallel(_ settings: USBExternalCamera.LiveStreamSettings) async throws {
    ...
    /// 스트림 설정 사전 준비
    private func prepareStreamSettingsQuickly(_ settings: USBExternalCamera.LiveStreamSettings) async throws {
    ...
    private func validateHaishinKitConfiguration() async -> ConfigValidationResult {
        ...
        if settings.streamKey.isEmpty {
            ...
            result.validItems.append("✅ 스트림 키: \(settings.streamKey.count)자 (앞 8자: \(String(settings.streamKey.prefix(8)))...)")
        }
    ...
    private func logDiagnosisReport(_ report: StreamingDiagnosisReport) {
        ...
        // 각 영역별 결과
        logger.info("📊 1️⃣ 설정 검증: \(report.configValidation.isValid ? "✅ 통과" : "❌ 실패") - \(report.configValidation.summary)", category: .system)
        logger.info("📊 2️⃣ MediaMixer: \(report.mediaMixerStatus.isValid ? "✅ 통과" : "❌ 실패") - \(report.mediaMixerStatus.summary)", category: .system)
        logger.info("📊 3️⃣ RTMPStream: \(report.rtmpStreamStatus.isValid ? "✅ 통과" : "❌ 실패") - \(report.rtmpStreamStatus.summary)", category: .system)
        logger.info("📊 4️⃣ 화면 캡처: \(report.screenCaptureStatus.isValid ? "✅ 통과" : "❌ 실패") - \(report.screenCaptureStatus.summary)", category: .system)
        logger.info("📊 5️⃣ 네트워크: \(report.networkStatus.isValid ? "✅ 통과" : "❌ 실패") - \(report.networkStatus.summary)", category: .system)
        logger.info("📊 6️⃣ 디바이스: \(report.deviceStatus.isValid ? "✅ 통과" : "❌ 실패") - \(report.deviceStatus.summary)", category: .system)
        logger.info("📊 7️⃣ 데이터 흐름: \(report.dataFlowStatus.isValid ? "✅ 통과" : "❌ 실패") - \(report.dataFlowStatus.summary)", category: .system)
    ...
    /// 하드웨어 최적화 결과 로깅
    private func logHardwareOptimizationResults(_ settings: USBExternalCamera.LiveStreamSettings) async {
        logger.info("📊 하드웨어 최적화 결과 요약:", category: .system)
        
        // 오디오 최적화 결과
        let audioQualityLevel = determineAudioQualityLevel(bitrate: settings.audioBitrate)
        logger.info("  🎵 오디오 품질 레벨: \(audioQualityLevel.rawValue) (\(settings.audioBitrate)kbps)", category: .system)
    ...
    /// 전체 최적화 상태 평가
    private func getOverallOptimizationStatus(settings: USBExternalCamera.LiveStreamSettings) -> String {
        ...
        // 오디오/비디오 품질 균형 평가
        let isBalanced = (audioLevel == .standard && videoPixels >= 1280*720 && videoPixels < 1920*1080) ||
    ...
    public func sendManualFrame(_ pixelBuffer: CVPixelBuffer) async {
        ...
        do {
            ...
            if successRate < 95.0 {
                logger.warning("📊 프레임 전송 성공률 낮음: \(String(format: "%.1f", successRate))% (\(frameTransmissionSuccess)/\(frameTransmissionCount))")
            }
    ...
    private func preprocessPixelBufferSafely(_ pixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
        do {
            ...
            // 스케일링 실행
            logger.info("🔄 해상도 스케일링 시작: \(currentWidth)x\(currentHeight) → \(targetWidth)x\(targetHeight)")
            
            guard let scaledBuffer = scalePixelBufferSafely(formatCompatibleBuffer, to: CGSize(width: targetWidth, height: targetHeight)) else {
                logger.error("❌ 해상도 스케일링 실패 - 포맷 변환된 버퍼 사용")
                return formatCompatibleBuffer
            }
            
            logger.info("🎉 해상도 스케일링 완료 및 검증 성공: \(CVPixelBufferGetWidth(scaledBuffer))x\(CVPixelBufferGetHeight(scaledBuffer))")
            return scaledBuffer
    ...
    /// 안전한 해상도 스케일링
    private func scalePixelBufferSafely(_ pixelBuffer: CVPixelBuffer, to targetSize: CGSize) -> CVPixelBuffer? {
    ...
    private func logVideoCodecDiagnostics(pixelBuffer: CVPixelBuffer) {
        ...
        logger.info("  - YUV420 포맷 여부: \(pixelFormat == kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange)")
    ...
    private func connectUsingExamplesPattern() {
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    self.currentStatus = .error(error as? LiveStreamError ?? LiveStreamError.streamingFailed(error.localizedDescription))
                }
            }
        }
    }
