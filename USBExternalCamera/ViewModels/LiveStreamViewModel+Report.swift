...
extension LiveStreamViewModel {
  // MARK: - Internal Methods - Report Generation
  func generateBasicInfoSection() -> String {
    var section = "📱 기본 정보:\n"
    section += "   • 앱 상태: \(status)\n"
  ...
  func generatePermissionSection() -> String {
  ...
  func generateDeviceSection() -> String {
  ...
  func generateYouTubeSection() async -> String {
  ...
  func generateRecommendationsSection() -> String {
    let cameraAuth = AVCaptureDevice.authorizationStatus(for: .video)
    let micAuth = AVCaptureDevice.authorizationStatus(for: .audio)
    var section = "💡 권장 사항:\n"
